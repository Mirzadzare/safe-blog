...
export default function UpdatePost() {
  ...
  useEffect(() => {
    if (editor && postForm.content && !editor.getHTML()) {
      editor.commands.setContent(postForm.content);
    }
  }, [editor, postForm.content]);
  ...
  const handleUpdate = async () => {
    try {
      ...
      // Only append new image if selected, otherwise keep existing
      if (selectedFile) {
        formData.append('image', selectedFile);
      } else if (postForm.image) {
        formData.append('existingImage', postForm.image);
      }
      ...
      setTimeout(() => navigate(`/post/${data.slug || postId}`), 1500);
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-600">Edit and update your post</p>
        </div>
